...
impl PlantMeshData {
    /// Export the plant mesh to Wavefront OBJ format with material references.
    /// Includes stem geometry and organ instance positions as point markers.
    /// The `mtl_filename` is referenced via `mtllib` so a matching MTL file
    /// should be written alongside the OBJ.
    pub fn to_obj(&self, mtl_filename: &str) -> String {
        let mut obj = String::new();
        obj.push_str("# Plant mesh generated by Apothecary's Satchel\n");
        obj.push_str(&format!("mtllib {mtl_filename}\n"));
    ...
    /// Generate a Wavefront MTL file with materials for each plant part.
    /// Colors are derived from the mesh instance data; stems use a default brown.
    pub fn to_mtl(&self) -> String {
        let mut mtl = String::new();
        mtl.push_str("# Plant materials generated by Apothecary's Satchel\n\n");

        // Stem material — default brown
        mtl.push_str("newmtl stem\n");
        mtl.push_str("Kd 0.40 0.26 0.13\n");
        mtl.push_str("Ka 0.10 0.07 0.03\n");
        mtl.push_str("Ks 0.10 0.10 0.10\n");
        mtl.push_str("Ns 10.0\n\n");

        // Leaf material
        if let Some(leaf) = self.leaf_instances.first() {
            let c = &leaf.color;
            mtl.push_str("newmtl leaf\n");
            mtl.push_str(&format!("Kd {:.4} {:.4} {:.4}\n", c.r, c.g, c.b));
            mtl.push_str(&format!("Ka {:.4} {:.4} {:.4}\n", c.r * 0.2, c.g * 0.2, c.b * 0.2));
            mtl.push_str("Ks 0.05 0.05 0.05\n");
            mtl.push_str("Ns 5.0\n\n");
        }

        // Flower material
        if let Some(flower) = self.flower_instances.first() {
            let c = &flower.color;
            mtl.push_str("newmtl flower\n");
            mtl.push_str(&format!("Kd {:.4} {:.4} {:.4}\n", c.r, c.g, c.b));
            mtl.push_str(&format!("Ka {:.4} {:.4} {:.4}\n", c.r * 0.2, c.g * 0.2, c.b * 0.2));
            mtl.push_str("Ks 0.20 0.20 0.20\n");
            mtl.push_str("Ns 20.0\n\n");
        }

        // Fruit material
        if let Some(fruit) = self.fruit_instances.first() {
            let c = &fruit.color;
            mtl.push_str("newmtl fruit\n");
            mtl.push_str(&format!("Kd {:.4} {:.4} {:.4}\n", c.r, c.g, c.b));
            mtl.push_str(&format!("Ka {:.4} {:.4} {:.4}\n", c.r * 0.2, c.g * 0.2, c.b * 0.2));
            mtl.push_str("Ks 0.15 0.15 0.15\n");
            mtl.push_str("Ns 15.0\n\n");
        }
